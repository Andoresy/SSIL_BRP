...
def train(log_path = None, dict_file = None):
    ...
    model = AttentionModel_LSTM(device=device, n_encode_layers=n_encode_layers,n_heads= n_heads, ff_hidden=ff_hidden, embed_dim=embed_dim, max_stacks = max_stacks, max_tiers = max_tiers)
    ...
    torch.save(dataloader, "uBRP_Imitation/train_data/train_data.pt")
    torch.save(val_dataloader, "uBRP_Imitation/train_data/val_data.pt")
    """ 
    
    dataloader = torch.load("uBRP_Imitation/train_data/train_data.pt")
    val_dataloader = torch.load("uBRP_Imitation/train_data/val_data.pt")
    ...
    def bc_loss(model, inputs, labels, t, device):
        ...
        #selected_ps = torch.gather(ps, dim=1, index=labels)
...
if __name__ == '__main__':
    ...
    #Create new path
    i = 0 
    newpath = f'uBRP_Imitation/train/Exp{i}' 
