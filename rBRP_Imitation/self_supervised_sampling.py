...
from data import data_from_caserta, data_from_caserta_for_greedy, generate_data
import torch
from Env import Env
import gc
...
class SSIL_baseline:
    ...
    def callback(self, device = 'cuda:0', model = None,  max_stacks = 6, max_tiers = 8, epoch=None):
        model.eval()
    ...
    def callback_reset(self, device = 'cuda:0', model = None,  max_stacks = 6, max_tiers = 8, epoch=None):
        model.eval()
        problems = self.problems
        if model is None:
            raise NotImplementedError
